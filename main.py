...
class SuslikFarmer:
    ...
    async def create_group_and_add_bot(self):
        ...
        try:
            # Создаем приватный канал (группу)
            result = await self.client(tl.functions.channels.CreateChannelRequest(
            ...
            await self.client(tl.functions.channels.InviteToChannelRequest(
                channel=channel,
                users=[bot]
            ))
    ...
    async def send_command_as_channel(self, channel, command: str):
        ...
        try:
            # Отправляем сообщение от имени канала
    ...
    async def delete_group(self, channel):
        """Удалить группу"""
        try:
            await self.client(tl.functions.channels.DeleteChannelRequest(channel=channel))
